...
export default function SupervisorShiftsPage() {
  ...
  const fetchShiftsAndAttendance = async () => {
    try {
      setLoading(true);
      const resAttendance = await fetch("/api/supervisor/attendance");
  ...
  return (
    <div className="w-full min-h-screen bg-slate-100 pb-24">
      ...
      <main className="max-w-md mx-auto px-4 mt-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 border border-slate-200 space-y-3">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                ...
                >
                  <span className={`text-[10px] font-bold font-mono ${isToday ? "text-amber-600 font-extrabold" : "text-slate-600"}`}>
                    {dayNum}
                  </span>
                  ...
                  {hasRecord ? (
                    <div className="flex items-center justify-center my-auto">
                      {isBothShifts ? (
                        <div className="px-1 py-0.5 bg-gradient-to-r from-amber-500 to-yellow-500 text-white rounded-full flex items-center gap-0.5 text-[8px] font-bold shadow-sm ring-2 ring-amber-300">
                          <span>☀️</span>
                          <span>🌙</span>
                        </div>
                      ) : hasNightShift ? (
                    ...
                    <span className="w-1.5 h-1.5 bg-amber-500 rounded-full mb-1 animate-ping"></span>
                  ) : null}
                </div>
              );
            })}
          ...
          <div className="grid grid-cols-4 text-center text-[9px] text-slate-500 pt-2 border-t gap-1">
            ...
            <span className="flex items-center justify-center gap-1">
              <span className="px-1 bg-gradient-to-r from-amber-500 to-yellow-500 text-white rounded-full text-[7px] font-bold">☀️🌙</span>
              <span>เช้า+ดึก</span>
            </span>
            <span className="flex items-center justify-center gap-1">
              ...
              {shifts.map((shift, index) => (
                <div key={index} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center text-xs">
                  <div className="space-y-1">
                    ...
                    <span className="text-amber-600 font-semibold block">{shift.shiftName}</span>
              ...
              {selectedDayDetail.records.map((rec, idx) => {
                ...
                return (
                  <div key={idx} className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 space-y-2 text-xs">
                    ...
                    <div className="p-2 bg-amber-50/80 rounded-xl border border-amber-200/80 text-[11px] flex items-center gap-1.5 text-amber-950 font-medium">
                      <span className="text-xs shrink-0">📍</span>
                      <span className="truncate">
                        หน่วยงาน: <strong className="text-amber-900 font-bold">{rec.location || userBranchName}</strong>
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-slate-700 bg-white p-2 rounded-xl border border-slate-100 font-mono text-[11px]">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-amber-600">🔴 {rec.checkOut || "-"}</span>
                      </div>
                    </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 px-2 py-2 flex justify-around items-center z-50 shadow-lg max-w-md mx-auto">
        ...
        <Link href="/supervisor/shifts" className="flex flex-col items-center text-amber-400 text-[10px] font-bold transition">
