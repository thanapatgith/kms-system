...
export default function SupervisorEquipmentPage() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/supervisor/equipment");
      ...
      <main className="max-w-md mx-auto px-4 mt-4 space-y-3">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 border border-slate-200 flex justify-between items-center gap-3">
          ...
          <button
            ...
            className="px-3.5 py-2 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl shadow-md transition cursor-pointer shrink-0"
          >
            + ยื่นคำขอใหม่
          </button>
        </div>
                ...
                <div key={item.id} className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 flex justify-between items-start text-xs space-y-1 shadow-sm">
                  <div className="space-y-1">
                    ...
                    <span className="text-[11px] text-slate-600 block">
                      จำนวน: <span className="font-bold text-amber-600">{item.quantity}</span> ชิ้น | สาเหตุ: {item.reasonType === "REPLACE" ? "ชำรุด/เสื่อมสภาพ" : item.reasonType === "LOST" ? "สูญหาย" : "พนักงานใหม่/ขอเพิ่ม"}
                    </span>
                    {item.reason && (
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-900 focus:bg-white focus:ring-2 focus:ring-amber-500 outline-none cursor-pointer"
                >
              ...
              <div className="flex justify-end gap-2 pt-2 border-t">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl shadow-md transition disabled:opacity-50 cursor-pointer"
                >
