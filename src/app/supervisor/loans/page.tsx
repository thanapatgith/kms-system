...
export default function SupervisorLoansPage() {
  ...
  // ⭐ ไม่ฮาร์ดโค้ด: เริ่มต้นด้วยค่า 0 เพื่อรอโหลดข้อมูลจริงจาก API Backend
  ...
  const fetchLoans = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/supervisor/loans");
      const data = await res.json();
      if (data.success) {
        setLoans(data.loans || []);
        // รับค่าคำนวณจริงจาก API Backend แบบไดนามิก 100%
        setLoanSummary({
          targetRound: data.targetRound || 20,
          isWindowOpen: data.isWindowOpen ?? false,
          workedDays: data.workedDays || 0,
          dailyWage: data.dailyWage || 560,
  ...
  const totalInterestAmount = loanSummary.totalBorrowedThisMonth >= 3000 
  ...
  return (
    <div className="w-full min-h-screen bg-slate-100 pb-32 text-base font-sans">
      ...
      <header className="bg-slate-900 text-white shadow-md sticky top-0 z-50 border-b border-slate-800">
        <div className="max-w-md mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-2.5">
            <span className="px-3 py-1 bg-amber-500 font-extrabold text-xs rounded-lg uppercase tracking-wider text-slate-950 shadow">
              SUPERVISOR
            </span>
       ...
       {/* การ์ดสรุปวงเงิน & สิทธิ์คงเหลือ */}
        <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white rounded-3xl p-5 shadow-xl space-y-4 border border-slate-700">
          ...
          <div className="bg-slate-800/60 px-4 py-3.5 rounded-2xl border border-slate-700/50 flex justify-between items-center">
            ...
            <div className="text-2xl sm:text-3xl font-black font-mono text-amber-400 tracking-tight flex items-center gap-1">
              <span>฿</span>
          ...
          <div className="grid grid-cols-3 gap-2.5 pt-2 border-t border-slate-700 text-center font-mono">
            <div className="bg-slate-900/90 p-2.5 rounded-xl border border-slate-800 shadow-sm">
              ...
              <span className="font-extrabold text-white text-sm">{loading ? "..." : `${loanSummary.workedDays} วัน`}</span>
            </div>
            <div className="bg-slate-900/90 p-2.5 rounded-xl border border-slate-800 shadow-sm">
              <span className="block text-xs font-sans text-slate-400 font-medium mb-0.5">วงเงินกู้เต็มสิทธิ์</span>
              <span className="font-extrabold text-white text-sm">{loading ? "..." : `฿${loanSummary.maxCredit.toLocaleString()}`}</span>
            </div>
            <div className="bg-slate-900/90 p-2.5 rounded-xl border border-slate-800 shadow-sm">
              <span className="block text-xs font-sans text-slate-400 font-medium mb-0.5">กู้ไปแล้วเดือนนี้</span>
              <span className="font-extrabold text-amber-400 text-sm">{loading ? "..." : `฿${loanSummary.totalBorrowedThisMonth.toLocaleString()}`}</span>
            </div>
          </div>
        </div>

        {/* ยื่นคำร้องกู้ยืมเงิน */}
        <div className="bg-white rounded-2xl shadow-sm p-5 border-2 border-slate-200 space-y-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500 font-medium">
              เปิดให้ยื่นเรื่องวันที่ {loanSummary.targetRound === 20 ? "11-17" : "21-27"} ของเดือนนี้
            </p>
          </div>
          ...
          <div className="w-full">
            {!loanSummary.isWindowOpen ? (
              ...
              <Link
                href="/supervisor/loans/create"
                className="w-full py-3.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-sm font-extrabold rounded-xl shadow-md transition text-center block"
              >
                + ยื่นเรื่องกู้ยืม
              </Link>
            )}
        ...
        {loanSummary.totalBorrowedThisMonth >= 3000 && (
          <div className="p-4 bg-amber-50 border-2 border-amber-200 rounded-2xl text-amber-900 text-xs space-y-2 shadow-sm">
            ...
            <p className="text-xs text-amber-900 leading-relaxed font-medium">
              ยอดกู้สะสมในเดือนนี้รวม <strong className="font-bold">฿{loanSummary.totalBorrowedThisMonth.toLocaleString()}</strong> (เข้าเกณฑ์ตั้งแต่ 3,000 บาทขึ้นไป)
            </p>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-1.5">
              <button
                ...
                className="px-2.5 py-1.5 bg-slate-100 hover:bg-amber-100 hover:text-amber-800 text-slate-800 text-xs font-bold rounded-xl border border-slate-200 transition cursor-pointer"
              >
                วันนี้
              </button>
              <button
          ...
          <div className="grid grid-cols-2 gap-2.5 text-xs">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-50 border-2 border-slate-200 rounded-xl text-slate-900 font-bold outline-none focus:bg-white focus:ring-2 focus:ring-amber-500 cursor-pointer text-sm"
              />
            </div>
            <div>
          ...
          {loading ? (
            ...
            <div className="space-y-3.5">
              {filteredLoans.map((item) => {
                ...
                const isOverInterestThreshold = loanSummary.totalBorrowedThisMonth >= 3000;
                  ...
                  <div key={item.id} className="p-4 bg-slate-50 rounded-2xl border-2 border-slate-200 space-y-3 text-sm shadow-sm">
                    ...
                    <div className="p-3 bg-white rounded-xl border border-slate-200 space-y-2 text-xs text-slate-700">
                      ...
                      <div className="flex justify-between text-xs font-extrabold text-slate-900 pt-1.5 border-t border-slate-100">
                        ...
                        <span className="font-mono text-amber-600 text-base">฿{totalDeduction.toLocaleString()}</span>
                      </div>
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 px-2 py-3 flex justify-around items-center z-50 shadow-lg max-w-md mx-auto">
        <Link href="/supervisor/dashboard" className="flex flex-col items-center text-slate-400 hover:text-amber-400 text-xs font-semibold transition">
          <span className="text-xl mb-0.5">📊</span>
          แดชบอร์ด
        </Link>
        <Link href="/supervisor/random-check" className="flex flex-col items-center text-slate-400 hover:text-amber-400 text-xs font-semibold transition">
          <span className="text-xl mb-0.5">📍</span>
          สุ่มตรวจ
        </Link>
        <Link href="/supervisor/logbook" className="flex flex-col items-center text-slate-400 hover:text-amber-400 text-xs font-semibold transition">
          <span className="text-xl mb-0.5">📋</span>
          Logbook
        </Link>
        <Link href="/supervisor/leaves" className="flex flex-col items-center text-slate-400 hover:text-amber-400 text-xs font-semibold transition">
          <span className="text-xl mb-0.5">📄</span>
          อนุมัติลา
        </Link>
      </nav>
