...
export default function SupervisorApplyLeavePage() {
  ...
  const fetchLeaves = async () => {
    try {
      setLoading(true);
      // ดึงประวัติการลาของ Supervisor (ส่งให้ HR/ผู้บริหารอนุมัติ)
      const res = await fetch("/api/supervisor/apply-leave");
  ...
  // คำนวณจำนวนวันลาสะสมในปีปัจจุบัน
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg("ยื่นใบลาสำเร็จ! ส่งเรื่องให้ HR/ผู้บริหารพิจารณาเรียบร้อย");
  ...
  return (
    <div className="w-full min-h-screen bg-slate-100 pb-24">
      ...
      <header className="bg-slate-900 text-white shadow-md sticky top-0 z-50">
        <div className="max-w-md mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-bold">ระบบลาผู้ควบคุมงาน</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-md mx-auto px-4 mt-4 space-y-3">
        ...
        <div className="bg-slate-900 text-white rounded-2xl p-4 shadow-lg flex justify-between items-center border border-slate-800">
          <div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 border border-slate-200 flex justify-between items-center">
          <div>
            <h2 className="text-sm font-bold text-slate-900">ยื่นคำขอลา (ส่วนตัว)</h2>
            <p className="text-[11px] text-slate-500 mt-0.5">อนุมัติโดย HR หรือผู้บริหาร</p>
          </div>
          <button
            onClick={() => setShowModal(true)}
            className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl shadow-md transition cursor-pointer"
          >
            + ยื่นใบลาใหม่
          </button>
        </div>
                ...
                <div key={leave.id} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center text-xs">
                  <div className="space-y-0.5">
                    <span className="font-bold text-slate-900 block text-sm">
                      ...
                      {leave.leaveType !== "SICK" && leave.leaveType !== "PERSONAL" && leave.leaveType !== "VACATION" && leave.leaveType}
                    </span>
                    <span className="text-[11px] text-slate-600 block">
                      {leave.startDate ? new Date(leave.startDate).toLocaleDateString("th-TH") : "-"} (ลา {leave.durationDays || 1} วัน)
                    </span>
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-900 focus:bg-white focus:ring-2 focus:ring-amber-500 outline-none"
                >
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:bg-white focus:ring-2 focus:ring-amber-500 outline-none cursor-pointer"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:bg-white focus:ring-2 focus:ring-amber-500 outline-none"
                  />
                </div>
              </div>

              {/* แจ้งเตือนสิทธิ์วันหยุดพิเศษ */}
                ...
                <p className="leading-tight">
                  {isOverYearlyLimit ? (
                    <strong className="text-red-600 block">
                      ยอดสะสมในปี {currentYear} จะเป็น {newTotalLeaveDays} วัน (ตั้งแต่ 3 วันขึ้นไป) ส่งผลต่อสิทธิ์พิเศษ
                    </strong>
                  ) : (
                    `ปัจจุบันลาไปแล้ว ${totalLeaveDaysThisYear} วัน`
                  )}
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:bg-white focus:ring-2 focus:ring-amber-500 outline-none resize-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl shadow-md transition disabled:opacity-40 cursor-pointer"
                >
                  {submitting ? "กำลังส่ง..." : "ยืนยันการยื่นลา (ส่ง HR)"}
                </button>
              </div>
            </form>
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 px-2 py-2 flex justify-around items-center z-50 shadow-lg max-w-md mx-auto">
        ...
        <Link href="/supervisor/apply-leave" className="flex flex-col items-center text-amber-400 text-[10px] font-bold transition">
